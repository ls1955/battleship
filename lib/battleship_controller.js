...
export class BattleShipController {
    ...
    placeShipsOnHumanBoard() {
        ...
        ships.forEach((ship, i) => {
            this.humanBoard.place({ ship, x: coors[i][0], y: coors[i][1] });
        });
    }
    ...
    // Update the DOM for the ending screen and remove every attackListener
    wrapUp({ winner }) {
        const h1 = document.querySelector("h1");
        h1.textContent = `The winner: ${winner}`;

        const columns = document.querySelectorAll(".comp-board .column");
        columns.forEach((c) => c.removeEventListener("click", this.atkEvent));
    }
}

/*
    Now that author think about it, why not just have a random button,
    where computer will place our ship for us?

    Pros:
    1. Its easy to implement. Add a damn button, and let computer player do it
       for us. Its also damn easy to disable/hide that button afterward
    Cons:
    1. Now the player have to believe in god of fate, a.k.a not having any control
       in placing their ship
    2. Have to implement GameBoard#clear, which is not bad at all
*/
